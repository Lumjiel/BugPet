...
本分支 (`optimize`) 是对原项目的重构优化，以下是计划中的优化方向：

### 1. 性能优化

| 优化项 | 目标 | 当前状态 |
|--------|------|----------|
| 状态轮询频率 | 降低 CPU 占用 | 需优化 |
| 图片资源 | 懒加载、按需加载 | 待实现 |
| 动画性能 | 使用 CSS transform 替代位置变化 | 待优化 |

### 2. 功能增强

| 功能 | 描述 | 优先级 |
|------|------|--------|
| **显示桌面检测** | Win+D 时切换专属语料 | ✅ 已实现 |
| 多显示器支持 | 宠物可移动到不同显示器 | 待开发 |
| 自定义宠物 | 用户可上传自己的宠物图片 | 待开发 |
| 插件系统 | 支持第三方语料包、动画包 | 待设计 |

### 3. 代码质量

| 方向 | 说明 |
|------|------|
| **模块化重构** | 将 App.vue 拆分为更小的模块 |
| **TypeScript 增强** | 完善类型定义，减少 any |
| **Rust 抽象** | 将 Windows API 调用封装为独立模块 |
| **测试覆盖** | 添加单元测试和集成测试 |
...
| 优化项 | 描述 |
|--------|------|
| **迁移到 Rust 端存储** | 使用 tauri-plugin-store 而非 localStorage |
| **数据导出/导入** | 用户可备份和恢复数据 |
| **云同步** | 可选的云端数据同步（待定） |
...
| 功能 | 描述 |
|------|------|
| **桌面状态语料** | ✅ 已实现 (desktop.ts) |
| **时间感知语料** | 早/中/晚不同时间段说不同话 |
| **天气联动** | 根据实际天气说相关的话 |
| **自定义语料** | 用户可编辑自己的语料库 |

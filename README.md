...
一款基于 **Tauri + Vue 3 + Rust** 构建的 Windows 桌面宠物应用，追踪你的编码状态，与你共同成长。
...
![Windows](https://img.shields.io/badge/Windows-Windows%2010/11-0078D4?style=flat-square)
...
> ⚠️ **本项目是 [macOS 原版 BugPet](https://github.com/ChenXin360/BugPet) 的 Windows 移植版本**，基于 Tauri + Vue 3 + Rust 重构，与原版技术栈不同。

作为一款 Windows 桌宠应用，BugPet 不仅提供陪伴体验，还能智能识别你的编码状态，通过可爱的宠物形象实时反馈你的工作模式。

### 与原版的区别

| 特性 | macOS 原版 | Windows 移植版 |
|------|------------|---------------|
| **技术栈** | Swift + AppKit | Tauri + Vue + Rust |
| **平台** | macOS | Windows 10/11 |
| **安装包** | .app / .dmg | .exe (NSIS) |
| **资源占用** | 较低 | 极低 (~20MB) |
...
| 功能 | 描述 |
|------|------|
| **四种宠物** | BugCat、TRAE、Codex、Claude Code |
...
| 功能 | 描述 |
|------|------|
| **显示桌面语料** | 按 Win+D 显示桌面时，宠物会说专属台词 |
| **透明窗口** | 真正透明的窗口，不遮挡桌面壁纸 |
| **极低资源占用** | 内存占用仅 ~20MB，不影响开发性能 |

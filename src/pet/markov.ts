...
class WeightedMarkovChain {
  ...
  private getWeightMultiplier(token: string): number {
    const highFreqMap: Record<PetState, string[]> = {
      ...
      desktop: ['桌面','干净','整理','壁纸','desktop','clean','organize']
    };
...
const desktopCorpus = [
  ...
  '桌面空空如也，你在思考人生吗喵？',
  '这么干净的桌面，我都不忍心搞乱了喵。',
  'The desktop is so clean, I dare not step on it meow.',
  'Nice wallpaper, can you change it to one with fish meow?',
  'So organized, you must be a neat freak meow.',
  'Empty desktop, are you contemplating life meow?',
  'Such a clean desktop, I hate to mess it up meow.'
];

const messageGenerators: Record<PetState, WeightedMarkovChain> = {
